...
class AttentionDynamic(nn.Module):
    """Calculates attention over the input nodes given the current state."""

    def __init__(self, hidden_size):
        super(AttentionDynamic, self).__init__()
    ...
    def forward(self, dynamic_hidden, decoder_hidden):

        batch_size, hidden_size, _ = dynamic_hidden.size()

        decoder_hidden = decoder_hidden.unsqueeze(2).expand_as(dynamic_hidden)

        hidden = decoder_hidden + dynamic_hidden
        # Broadcast some dimensions so we can do batch-matrix-multiply
        W = self.W.expand(batch_size, 1,hidden_size )
...
class PointerDynamic(nn.Module):
    """Calculates the next state given the previous state and input embeddings."""

    def __init__(self, hidden_size, num_layers=1, dropout=0.1):
        super(PointerDynamic, self).__init__()

        self.hidden_size = hidden_size

        # Used to compute a representation of the current decoder output
        self.lstm = torch.nn.LSTMCell(input_size=hidden_size, hidden_size = hidden_size, device=device)
        self.encoder_attn = AttentionDynamic(hidden_size).to(device)
    ...
    def forward(self, dynamic_hidden, decoder_hidden, last_hh,last_cc):
        ...
        enc_attn = self.encoder_attn(dynamic_hidden, last_hh_1)
...
class DRL4MetroDynamic(nn.Module):  
    """Defines the main Encoder, Decoder, and Pointer combinatorial models.
    ...
    def __init__(self, dynamic_size, hidden_size, num_layers=1, dropout=0., update_fn = None, mask_fn = None, v_to_g_fn = None,
                 vector_allow_fn = None):
        super(DRL4MetroDynamic, self).__init__()
        ...
        # self.static_encoder = Encoder(static_size, hidden_size)
        self.dynamic_encoder = Encoder(dynamic_size, hidden_size)
        self.decoder = Encoder(dynamic_size, hidden_size)
        self.pointer = PointerDynamic(hidden_size, num_layers, dropout)
        ...
        # Used as a proxy initial state in the decoder when not specified
        self.x0 = torch.zeros((1, dynamic_size, 1), requires_grad=True, device=device)


    def forward(self, dynamic, station_num_lim, budget=None, initial_direct = None,line_unit_price = None, station_price = None,
                decoder_input=None, last_hh=None):
        # initial_direct: direction 
        # line_unit_price: example:  1.0
        # station_price: example: 2.0
        ...
        batch_size, input_size, sequence_size = dynamic.size()
        ...
        for _ in range(max_steps):
            ...
            probs, last_hh,last_cc  = self.pointer(dynamic_hidden,
                                          decoder_hidden, last_hh,last_cc)
            ...
            decoder_input = torch.gather(dynamic, 2,
                                         ptr.view(-1, 1, 1)
                                         .expand(-1, input_size, 1)).detach()
            #decoder_input: 
            # budget
