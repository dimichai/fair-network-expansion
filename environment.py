...
class Environment(object):
    ...
    def station_distance_matrix(self, tour_idx):
        # Transform station numbers to grid coordinates and calculate all the direct distances 
        # for all stations
    ...
    def __init__(self, env_path, groups_file=None, reward_scaling_fn=None, efficient_station_fn=None, 
        ...
        if reward_scaling_fn == "linear":
            c = 1
            self.reward_scaling_fn = lambda d_eucl, d_tour: torch.nn.functional.relu(1 - ((d_tour/d_eucl)-1)/c)
        elif reward_scaling_fn == "parabolic":
            c = 1
            self.reward_scaling_fn = lambda d_eucl, d_tour: torch.sqrt(torch.nn.functional.relu(1 - ((d_tour/d_eucl)-1)/c))
        ...
        if efficient_station_fn == None:
            ...
            print("The use of the efficient station function is discouraged. To enable it, please change the `cf_reward` function.")
